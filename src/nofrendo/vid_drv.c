...
static int vid_findmode(int width, int height, viddriver_t *osd_driver)
{
   ...
   nof_log_printf("screen2 : %d x %d\n", screen->width, screen->height);

   /* release surface */
   if (driver->free_write)
      driver->free_write(-1, NULL);

   nof_log_printf("screen3 : %d x %d\n", screen->width, screen->height);

   nof_log_printf("video driver: %s at %dx%d\n", driver->name,
              screen->width, screen->height);
...
int vid_init(int width, int height, viddriver_t *osd_driver)
{
   ...
   {
      nof_log_printf("video initialization failed for %s at %dx%d\n",
                 osd_driver->name, width, height);
      return -1;
   }
	nof_log_printf("vid_init done\n");
