      ...
      while ((line = my_getline(config_file)))
      {
         ...
         do 
         {
            ...
            case '[':
               ...
               if (NULL == s)
               {
                  nof_log_printf("load_config: missing ']' after group\n");
            ...
            default:
               ...
               if (NULL == s)
               {
                  nof_log_printf("load_config: missing '=' after key\n");
                  s = key + strlen(key);
               }
               else
               {
...

               {
                  ...
                  if (NULL == var)
                  {
                     nof_log_printf("load_config: my_create failed\n");
                     return -1;
                  }
...
static int save_config(char *filename)
{
   ...
   if (NULL == config_file)
   {
      nof_log_printf("save_config failed\n");
      return -1;
   }
...
static void write_int(const char *group, const char *key, int value)
{
   ...
   if (NULL == var)
   {
      nof_log_printf("write_int failed\n");
      return;
   }
...
static void write_string(const char *group, const char *key, const char *value)
{
   ...
   if (NULL == var)
   {
      nof_log_printf("write_string failed\n");
      return;
   }
