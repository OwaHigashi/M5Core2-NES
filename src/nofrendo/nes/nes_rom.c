...
static void rom_savesram(rominfo_t *rominfo)
{
   ...
   if (rominfo->flags & ROM_FLAG_BATTERY)
   {
      ...
      if (NULL != fp)
      {
         ...
         fclose(fp);
         nof_log_printf("Wrote battery RAM to %s.\n", fn);
      }
   }
}
...
static void rom_loadsram(rominfo_t *rominfo)
{
   ...
   if (rominfo->flags & ROM_FLAG_BATTERY)
   {
      ...
      if (NULL != fp)
      {
         ...
         fclose(fp);
         nof_log_printf("Read battery RAM from %s.\n", fn);
      }
   }
}
...
//      fread(rominfo->sram + TRAINER_OFFSET, TRAINER_LENGTH, 1, fp);
      ...
      rom+=TRAINER_LENGTH;
      nof_log_printf("Read in trainer at $7000\n");
   }
}
...
static void rom_checkforpal(rominfo_t *rominfo)
{
   ...
   nof_log_printf("Game specific palette found -- assuming VS. UniSystem\n");
}
...
//   _fread(&head, 1, sizeof(head), fp);
   ...
   else
   {
      ...
      if (('D' == head.mapper_hinybble) && (0 == memcmp(head.reserved, "iskDude!", 8)))
         nof_log_printf("`DiskDude!' found in ROM header, ignoring high mapper nybble\n");
      else
      {
         nof_log_printf("ROM header dirty, possible problem\n");
         rominfo->mapper_number |= (head.mapper_hinybble & 0xF0);
      }
...
void rom_free(rominfo_t **rominfo)
{
   ...
   if ((*rominfo)->flags & ROM_FLAG_VERSUS)
   {
      ...
      nof_log_printf("Default NES palette restored\n");
   }
   ...
   if ((*rominfo)->sram)
      free((*rominfo)->sram);
   /* rom and vrom point into osd_getromdata() buffer, not individually malloc'd */
   (*rominfo)->rom = NULL;
   (*rominfo)->vrom = NULL;
   if ((*rominfo)->vram)
      free((*rominfo)->vram);
