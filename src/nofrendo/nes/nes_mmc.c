...
void mmc_bankvrom(int size, uint32 address, int bank)
{
   ...
   default:
      nof_log_printf("invalid VROM bank size %d\n", size);
   }
}
...
void mmc_bankrom(int size, uint32 address, int bank)
{
   ...
   default:
      nof_log_printf("invalid ROM bank size %d\n", size);
      break;
   }
...
{
   nof_log_printf("setting up mapper %d\n", mmc.intf->number);
...
void mmc_reset(void)
{
   ...
   nof_log_printf("reset memory mapper\n");
}
...
mmc_t *mmc_create(rominfo_t *rominfo)
{
   ...
   nof_log_printf("created memory mapper: %s\n", (*map_ptr)->name);
